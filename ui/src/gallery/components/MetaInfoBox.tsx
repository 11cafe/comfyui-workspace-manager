...
import {Flex, IconButton, Link, SimpleGrid, Text, Tooltip} from '@chakra-ui/react'
import {IconDownload} from '@tabler/icons-react'
import {formatTimestamp} from '../../utils.tsx'
import {isMultiKSampler, MultiKSampler} from './MetaBoxByType/MultiKSampler.tsx'
...
export const MetaInfoBox = ({media}: {media?: Media}) => {
  ...
  const getMetaData = async (curMedia: Media) => {
    try {
      const res = await getMetadataFromUrl(`/workspace/view_media?filename=${curMedia.localPath}`)
  ...
  return <Flex overflowY={'auto'} mb={4} direction={'column'} gap={2} flex={1}>
    ...
    {(() => {
      ...
      return isMultiKSampler(mediaMetaData) && <MultiKSampler {...p} />
      || <OneKSampler {...p} />
    })()}
  </Flex>
}